...
namespace exec::__system_context_backend {
  ...
  template <class _BaseSchedulerContext>
  struct __system_scheduler_impl : system_scheduler {
    __system_scheduler_impl()
      : __scheduler_(__base_context_.get_scheduler()) {
    }
   private:
    using __scheduler_t = decltype(std::declval<_BaseSchedulerContext>().get_scheduler());

    /// The underlying libdispatch queue.
    _BaseSchedulerContext __base_context_;
    /// The scheduler that will be used to schedule work.
    ...
    using __schedule_operation_t =
      __operation<decltype(stdexec::schedule(std::declval<__scheduler_t>()))>;

    using __bulk_schedule_operation_t = __operation<decltype(stdexec::bulk(
      stdexec::schedule(std::declval<__scheduler_t>()),
   ...
   public:
    void schedule(storage __storage, receiver* __r) noexcept override {
      try {
        auto __sndr = stdexec::schedule(__scheduler_);
        auto __os =
    ...
    void bulk_schedule(
      ...
      try {
        auto __sndr =
          stdexec::bulk(stdexec::schedule(__scheduler_), __size, __bulk_functor{__item_r});
        auto __os =
  ...
  void* __libdispatch_query_system_context_interface(std::type_index __id) noexcept {
